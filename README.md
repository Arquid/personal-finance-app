...
- **Overview dashboard**
  ...
  - Cash flow forecast chart — projects the next 30 days' balance from active recurring bills not yet paid this cycle (applied on their due day, or immediately if already overdue), the average of the last 3 months' income (applied on the day it typically arrives), and a smoothed daily average of everyday non-recurring spending
...
- Prisma is pinned to `6.x` rather than the current `7.x` major release — v7 switched to a TypeScript/ESM-only generated client and driver adapters, which doesn't fit this project's plain JavaScript/CommonJS stack. `npm audit` flags a high-severity issue in the `prisma` CLI's `@prisma/config` → `deepmerge-ts` chain, but `prisma` is a devDependency only (the CLI used for migrations/seeding) — the runtime `@prisma/client` dependency doesn't pull it in, so this doesn't reach the deployed server. Left unpatched deliberately rather than force-upgrading past the pinned major version for a dev-tooling-only exposure.
